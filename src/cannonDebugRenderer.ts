...
export default class CannonDebugRenderer {
	...
	private _typeMatch(mesh: THREE.Mesh | THREE.Points, shape: CANNON.Shape): boolean {
		...
		return (geo instanceof THREE.SphereGeometry && shape instanceof CANNON.Sphere) || (geo instanceof THREE.BoxGeometry && shape instanceof CANNON.Box) || (geo instanceof THREE.CylinderGeometry && shape instanceof CANNON.Cylinder) || (geo instanceof THREE.PlaneGeometry && shape instanceof CANNON.Plane) || shape instanceof CANNON.ConvexPolyhedron || (geo.id === shape.id && shape instanceof CANNON.Trimesh) || (geo.id === shape.id && shape instanceof CANNON.Heightfield);
	}
	...
	private _createMesh(shape: CANNON.Shape): THREE.Mesh | THREE.Points {
		...
		switch (shape.type) {
			...
			case CANNON.Shape.types.CYLINDER:
				geometry = new THREE.CylinderGeometry((shape as CANNON.Cylinder).radiusTop, (shape as CANNON.Cylinder).radiusBottom, (shape as CANNON.Cylinder).height, (shape as CANNON.Cylinder).numSegments);
			...
			case CANNON.Shape.types.TRIMESH:
				...
				for (let i = 0; i < (shape as CANNON.Trimesh).vertices.length; i += 3) {
					points.push(new THREE.Vector3((shape as CANNON.Trimesh).vertices[i], (shape as CANNON.Trimesh).vertices[i + 1], (shape as CANNON.Trimesh).vertices[i + 2]));
				}
				geometry.setFromPoints(points);
				mesh = new THREE.Mesh(geometry, material);
